CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
 ...
 "In TRIAL environment we cannot really influence / implement real authorization. Therefore we simulate authorization
 "with help of below helper methods.
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD acceptTravel.
    ...
    "Set the new travel status to 'Accepted'
  ...
  METHOD recalcTotalPrice.
    ...
    "write back the modified total price of travels
    MODIFY ENTITIES OF zi_btc_travel IN LOCAL MODE
      ENTITY Travel
  ...
  METHOD rejectTravel.
    ...
    "Set the new travel status to 'Cancelled'
    MODIFY ENTITIES OF ZI_BTC_Travel IN LOCAL MODE
      ENTITY Travel
        UPDATE
          FIELDS ( TravelStatus )
  ...
  METHOD calculateTotalPrice.
    ...
    MODIFY ENTITIES OF zi_btc_travel  IN LOCAL MODE
      ENTITY Travel
        EXECUTE recalcTotalPrice          "call Internal Action recalcTotalPrice
  ...
  METHOD setInitialStatus.
    ...
    "Read relevant instance of travel data
    READ ENTITIES OF zi_btc_travel IN LOCAL MODE
      ENTITY Travel
      FIELDS ( TravelStatus ) WITH CORRESPONDING #( keys )
      RESULT DATA(travels).

    "Remove records where status is already filled / set
  ...
  METHOD calculateTravelID.
    ...
    " The key of this business object is a UUID, calculated / generated by the framework.
  ...
  METHOD validateAgency.
    ...
    READ ENTITIES OF ZI_BTC_Travel IN LOCAL MODE
     ...
     "Optmization of DB select: extract distinct non-initial agency IDs
     ...
     "Raise msg for non-existing or Initial agency ID
     LOOP AT travels INTO DATA(travel).
       ...
       IF travel-AgencyID IS INITIAL OR NOT line_exists(  agencies_db[ agency_id = travel-AgencyID ] ).
         ...
         APPEND VALUE #( %tky         = travel-%tky
                         %state_area  = 'VALIDATE_AGENCY'
                         %msg         = NEW zcm_rap_btc( severity = if_abap_behv_message=>severity-error
                                                         textid   = zcm_rap_btc=>agency_unknown
  ...
  METHOD validateCustomer.
    ...
    "Read the relevant instance data
    READ ENTITIES OF zi_btc_travel IN LOCAL MODE
      ENTITY Travel
      FIELDS ( CustomerID )
      WITH CORRESPONDING #( keys )
    ...
    "optimization of DB select: extract non-initial distinct customer ids
    DATA customers TYPE SORTED TABLE OF /dmo/customer WITH UNIQUE KEY customer_id.

    customers = CORRESPONDING #( travels DISCARDING DUPLICATES MAPPING customer_id =  CustomerID EXCEPT * ).

    "Collect all the available customer IDs from DB (corresponding to the current instance)
    ...
    "Raise msg where customer ID is initial or non-existing in DB
    LOOP AT travels INTO DATA(travel).
      "Clear previous state messages that might exist
      APPEND VALUE #( %tky = travel-%tky
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
         ...
         APPEND VALUE #( %tky   = travel-%tky
                         %state_area = 'VALIDATE_CUSTOMER'
                         %msg  = NEW zcm_rap_btc( severity = if_abap_behv_message=>severity-error
                                                  textid   = zcm_rap_btc=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-BeginDate > travel-EndDate.
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %state_area = 'VALIDATE_DATES'
                        %msg     = NEW zcm_rap_btc( severity = if_abap_behv_message=>severity-error
                                                    textid   = zcm_rap_btc=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky = travel-%tky
                        %state_area = 'VALIDATE_DATES'
                        %msg     = NEW zcm_rap_btc( severity = if_abap_behv_message=>severity-error
                                                    textid   = zcm_rap_btc=>begin_date_before_system_date
                                                    begindate = travel-BeginDate )
  ...
  METHOD get_instance_authorizations.
    ...
    "For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish
    SELECT FROM zbtc_travel
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.  "Unauthorized
            ...
            APPEND VALUE #( %tky  = travel-%tky
                            %msg  = new zcm_rap_btc( severity = if_abap_behv_message=>severity-error
                                                     textid   = zcm_rap_btc=>unauthorized )
                          ) TO reported-travel.
          ENDIF.

        ELSE. "Creation of new record (since no before image found) -> check create authorization
      ...
      IF is_delete_requested = abap_true.
        ...
        IF delete_granted = abap_false.  "Unauthorized
            APPEND VALUE #( %tky = travel-%tky
                            %fail-cause = if_abap_behv=>cause-unauthorized ) TO failed-travel.
  ...
  METHOD is_create_granted.
    AUTHORITY-CHECK OBJECT 'ZOSTAT_BTC'
      ID 'ZOSTAT_BTC' DUMMY
      ID 'ACTVT' FIELD '01'.

    create_granted = COND #( WHEN sy-subrc = 0 THEN abap_true ELSE abap_false ).

    "Simulate full access - for testing purpose only!!
  ...
  METHOD is_delete_granted.
    IF has_before_image = abap_true.
        AUTHORITY-CHECK OBJECT 'ZOSTAT_BTC'
          ID 'ZOSTAT_BTC' FIELD travel_status
          ID 'ACTVT' FIELD '06'.
    ELSE.
        AUTHORITY-CHECK OBJECT 'ZOSTAT_BTC'
          ID 'ZOSTAT_BTC' DUMMY
